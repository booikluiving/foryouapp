...
function normalizePathEdge(input = {}) {
  ...
  const edgeType = normalizePathEdgeType(src.edgeType || src.type);
  return edgeType ? { fromSceneId, toSceneId, edgeType } : { fromSceneId, toSceneId };
}

function normalizePathEdgeType(value) {
  const key = String(value || "").trim().toLowerCase();
...
function isLoopPathEdge(edge = {}) {
  return normalizePathEdgeType(edge && edge.edgeType) === "loop";
}
...
function effectiveEndSceneIds(path = {}, sceneIds = null, edges = null) {
  ...
  return normalizeIdList(safePath.endSceneIds || []).filter((sceneId) => sceneSet.has(sceneId));
}
...
function normalizeCrossingThresholdsForPaths(thresholds = [], paths = []) {
  const byScene = new Map();
  for (const threshold of normalizeCrossingThresholds(thresholds)) {
    ...
    if (incomingPathCount <= 1) continue;
    if (incomingCount <= 1) continue;
    const requiredCount = clampInt(threshold.requiredCount, 1, incomingCount, incomingCount);
    if (requiredCount >= incomingCount) continue;
...
function validateAlgorithmPath(path = {}, { scenes = [] } = {}) {
  ...
  const nonLoopIncoming = new Map(safePath.sceneIds.map((sceneId) => [sceneId, []]));
  for (const edge of pathEdges) {
    ...
    if (!isLoopPathEdge(edge)) {
      if (nonLoopIncoming.has(edge.toSceneId)) nonLoopIncoming.get(edge.toSceneId).push(edge.fromSceneId);
  ...
  const canReachWithoutLoops = (startSceneId, targetSceneId) => {
    const stack = [startSceneId];
    const seen = new Set();
    while (stack.length) {
      const current = stack.pop();
  ...
  for (const edge of pathEdges.filter(isLoopPathEdge)) {
    if (!(nonLoopIncoming.get(edge.toSceneId) || []).length) issues.push(`path_loop_start_target:${edge.toSceneId}`);
    if (!canReachWithoutLoops(edge.toSceneId, edge.fromSceneId)) issues.push(`path_loop_direction:${edge.fromSceneId}`);
  }
...
function buildPathSceneStatuses(paths = [], sceneById = new Map(), played = new Set(), crossingThresholds = []) {
  ...
  const analysisByPathId = new Map(analyses.map((analysis) => [analysis.path.id, analysis]));
  ...
  for (const status of statusBySceneId.values()) {
    ...
    if (crossingRoutes.length > 1 && crossingRoutePathCount > 1) {
      ...
      const requiredCount = clampInt(
        crossingThresholdBySceneId.get(status.sceneId) || crossingRoutes.length,
        1,
        crossingRoutes.length,
        crossingRoutes.length
      );
      const crossingSatisfied = completedRoutes.length >= requiredCount || playedSet.has(status.sceneId);
      const missingRouteSourceIds = uniqueNormalizedIds(crossingRoutes
      ...
      status.predecessorIds = uniqueNormalizedIds(status.predecessorIds.concat(crossingRoutes.map((route) => route.fromSceneId)));
      status.requiredPredecessorIds = uniqueNormalizedIds(status.requiredPredecessorIds.concat(crossingRoutes.map((route) => route.fromSceneId)));
      status.completedPredecessorIds = uniqueNormalizedIds(status.completedPredecessorIds.concat(completedRoutes.map((route) => route.fromSceneId)));
      if (!crossingSatisfied && !playedSet.has(status.sceneId)) {
        status.missingPredecessorIds = uniqueNormalizedIds(status.missingPredecessorIds.concat(missingRouteSourceIds));
        status.requiredMissingPredecessorIds = uniqueNormalizedIds(status.requiredMissingPredecessorIds.concat(missingRouteSourceIds));
...
function buildAlgorithmOrder({ scenes = [], runs = [], settings = {}, catalog = null, preparedNext = null, lockedQueue = [] } = {}) {
  ...
  const pathBlockForScene = (scene) => {
    ...
    const blockingCrossingThreshold = status.blockingCrossingThreshold && !status.blockingCrossingThreshold.satisfied
      ...
      : null;
    const crossingMissing = blockingCrossingThreshold
      ? normalizeIdList((blockingCrossingThreshold.routes || [])
        .filter((route) => !route.completed)
        .map((route) => route.fromSceneId))
      : [];
    const effectiveMissing = normalizeIdList(missing.concat(crossingMissing));
    ...
    return {
      pathStatus: status,
      missingPredecessorIds: effectiveMissing,
      reason: expiredSide.length
        ...
        : blockingCrossingThreshold
        ? `Wacht op ${blockingCrossingThreshold.requiredCount} van ${blockingCrossingThreshold.incomingCount} routes naar deze kruising; ${blockingCrossingThreshold.completedCount} gespeeld.`
        : blockingThreshold
