...
async function main() {
  ...
  try {
    ...
    const storeAfterUploads = JSON.parse(await fs.readFile(TEST_STORE_PATH, "utf8"));
    ...
    const storeAfterOutputs = JSON.parse(await fs.readFile(TEST_STORE_PATH, "utf8"));
    ...
    const storeAfterDelete = JSON.parse(await fs.readFile(TEST_STORE_PATH, "utf8"));
