...
async function main() {
  ...
  const child = spawn(process.execPath, [path.resolve(TEST_DIR, "../server/server.js")], {
    ...
    env: {
      ...
      V2_CATALOG_STORE_PATH: TEST_STORE_PATH,
  ...
  try {
    ...
    const storeAfterReplacement = JSON.parse(await fs.readFile(TEST_STORE_PATH, "utf8"));
    ...
    const imageResponse = await fetch(`${BASE_URL}/v0/catalog/media-assets/file/${encodeURIComponent(replacementUpload.asset.id)}`);
    ...
    const audioResponse = await fetch(`${BASE_URL}/v0/catalog/media-assets/file/${encodeURIComponent(audioUpload.asset.id)}`);
