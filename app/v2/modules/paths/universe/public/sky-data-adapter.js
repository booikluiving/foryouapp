(function () {
  ...
  function runtimeOverlay(runtime) {
    ...
    const blockedSceneIds = new Set((runtime && runtime.blockedSceneIds || []).map((sceneId) => numberValue(sceneId)).filter(Boolean));
    const lockedSceneIds = new Set((runtime && runtime.lockedSceneIds || []).map((sceneId) => numberValue(sceneId)).filter(Boolean));
  ...
  function statusForScene(sceneId, runtime) {
    ...
    if (runtime.blockedSceneIds && runtime.blockedSceneIds.has(safeSceneId)) return "blocked";
    if (runtime.lockedSceneIds && runtime.lockedSceneIds.has(safeSceneId)) return "locked";
    return "";
  }
