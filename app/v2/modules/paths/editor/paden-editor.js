(() => {
  ...
  const LayoutRelaxer = window.PadenLayoutRelaxer;
  const TOKEN_KEY = "paths_editor_token_v2";
  const LAYOUT_KEY = "paden_editor_layouts_v2";
  const PREF_KEY = "paden_editor_prefs_v2";
  ...
  const LAYOUT_ANIMATION_MS = 1350;
  ...
  let testOverrideSceneIds = new Set();
  let testResultCache = null;
  let compositionTransformCache = {};
  let previousNodePositions = new Map();
  let previousGhostPositions = new Map();
  let suppressLayoutAnimation = false;
  let canvasScrollAnimation = 0;
  let edgeAnimationFrame = 0;
  ...
  function loadLocal() {
    ...
    try {
      ...
      pathsBarExpanded = !!prefs.pathsBarExpanded;
  ...
  function getTestResult() {
    ...
    const statuses = Graph.buildPathSceneStatuses({
      ...
      seedSceneIds: Array.from(testOverrideSceneIds),
  ...
  async function loadState({ show = false } = {}) {
    try {
      const data = await api("/v0/paths/editor-state");
  ...
  function relaxOffsetForScene(model, sceneId) {
    const id = Number(sceneId || 0);
    return model && model.relaxOffsets && model.relaxOffsets.get(id)
      ? model.relaxOffsets.get(id)
      : { x: 0, y: 0 };
  }

  function canvasToPathLocalPosition(model, pathOrKey, position = {}, sceneId = 0) {
    const transform = transformForPath(model, pathOrKey);
    const relaxOffset = relaxOffsetForScene(model, sceneId);
    return {
      x: Number(position.x || 0) - Number(transform.dx || 0) - Number(relaxOffset.x || 0),
      y: Number(position.y || 0) - Number(transform.dy || 0) - Number(relaxOffset.y || 0),
    };
  }
  ...
  function buildUnifiedCanvasModel() {
    ...
    const baseNodes = worldNodes.map((node) => ({
      ...node,
    ...
    const basePositions = Object.fromEntries(baseNodes.map((node) => [node.sceneId, node.position]));
    const relaxed = LayoutRelaxer && typeof LayoutRelaxer.relaxLayout === "function"
      ? LayoutRelaxer.relaxLayout({
        nodes: baseNodes,
      ...
      : {
        ...
        relaxOffsets: new Map(baseNodes.map((node) => [Number(node.sceneId), { x: 0, y: 0 }])),
      };
    const nodes = relaxed.nodes || baseNodes;
    const displayPathPositions = relaxed.pathPositions || renderPathPositions;
    ...
    return {
      paths,
      nodes,
      pathPositions: displayPathPositions,
      pathEdges,
      pathEndpoints,
      pathTransforms: renderTransforms,
      worldOffset: { x: offsetX, y: offsetY },
      baseLayout: { positions: basePositions },
      relaxOffsets: relaxed.relaxOffsets || new Map(),
      relaxEnabled: !!relaxed.enabled,
      layout,
    };
  }
  ...
  function renderChips() {
    const bar = $("chipbar");
    const paths = activePaths();
    const pathQuery = pathSearchQ.trim().toLowerCase();
    const visiblePaths = pathQuery
      ? paths.filter((path) => String(path && path.name || "").toLowerCase().includes(pathQuery))
      : paths;
    ...
    const chips = visiblePaths.map((path) => {
      const key = pathKey(path);
    ...
    bar.classList.toggle("compact", !pathsBarExpanded);
    bar.classList.toggle("expanded", !!pathsBarExpanded);
    bar.innerHTML = `
      ...
      ${chips || `<span class="chip">${pathQuery ? "Geen paden gevonden" : "Nog geen paden"}</span>`}
      <span class="barHint">${pathQuery ? `${visiblePaths.length}/${paths.length} gevonden` : `${selectedCount}/${paths.length} zichtbaar`}</span>
      <span class="chipbarSpacer"></span>
      <button type="button" class="chipAction chipbarToggle" data-toggle-chipbar title="${pathsBarExpanded ? "Paden inklappen" : "Alle paden uitklappen"}" aria-label="${pathsBarExpanded ? "Paden inklappen" : "Alle paden uitklappen"}" aria-expanded="${pathsBarExpanded ? "true" : "false"}">${pathsBarExpanded ? "▴" : "▾"}</button>
    `;

    const toggleChipbarBtn = bar.querySelector("[data-toggle-chipbar]");
    if (toggleChipbarBtn) toggleChipbarBtn.addEventListener("click", () => {
      pathsBarExpanded = !pathsBarExpanded;
      savePrefs();
      renderChips();
    });
  ...
  function canOverrideTestStatus(status) {
    const value = String(status && status.nodeStatus || "");
    return !!status && value !== "Played" && value !== "Available";
  }
  ...
  function undoTestStep() {
    const removed = testPlayedSceneIds.pop();
    testOverrideSceneIds.delete(Number(removed || 0));
    invalidateTestResult();
    renderCanvas();
  }
  ...
  function overrideTestScene(sceneId) {
    const id = Number(sceneId || 0);
    if (!testMode || !id) return;
    const status = testStatusFor(id);
    if (!status) {
  ...
  function handleTestNodeClick(sceneId) {
    const id = Number(sceneId || 0);
    if (!testMode || !id) return;
    const status = testStatusFor(id);
    if (status && status.nodeStatus === "Available") {
      playTestScene(id);
      return;
    }
  ...
  function renderTestPanel(model, testResult) {
    ...
    $("testTimeline").innerHTML = testPlayedSceneIds.length
      ? testPlayedSceneIds.map((id, index) => {
        const override = testOverrideSceneIds.has(Number(id));
        return `<span class="${override ? "override" : ""}"><b>${index + 1}</b>${esc(sceneTitle(id))}${override ? '<em>override</em>' : ""}</span>`;
      }).join("")
  ...
  function renderCanvas() {
    ...
    if (!targetPath || !model.paths.length) {
      ...
      previousGhostPositions = new Map();
    ...
    const edgeAnimation = buildEdgeAnimationState(model, ghosts);
    renderEdges(svg, model, ghosts, testResult, edgeAnimation ? edgeRenderOptions(edgeAnimation, 0) : {});
    ...
    startEdgeAnimation(svg, model, ghosts, testResult, edgeAnimation);
    renderMiniMap(model, ghosts);
    renderTestPanel(model, testResult);
    renderInspector(model, testResult);
    prunePositionCache(previousNodePositions, model.nodes.map((node) => positionKey("node", node.sceneId)));
    prunePositionCache(previousGhostPositions, ghosts.map(ghostAnimationKey));
    suppressLayoutAnimation = false;
  }
  ...
  function renderEdges(svg, model, ghosts, testResult = null, renderOptions = {}) {
    const body = [];
    const resolveNodePosition = typeof renderOptions.nodePosition === "function"
      ? renderOptions.nodePosition
      : (sceneId, position) => position;
    const resolveGhostPosition = typeof renderOptions.ghostPosition === "function"
      ? renderOptions.ghostPosition
      : (ghost, index, position) => position;
    ...
    (model.paths || []).forEach((path) => {
      ...
      edges.forEach((edge) => {
        const finalFrom = positions[edge.fromSceneId] || model.layout.positions[edge.fromSceneId];
        const finalTo = positions[edge.toSceneId] || model.layout.positions[edge.toSceneId];
        const from = resolveNodePosition(edge.fromSceneId, finalFrom) || finalFrom;
        const to = resolveNodePosition(edge.toSceneId, finalTo) || finalTo;
        if (!from || !to) return;
    ...
    ghosts.forEach((ghost, index) => {
      const finalFrom = model.layout.positions[ghost.fromSceneId];
      if (!finalFrom) return;
      const from = resolveNodePosition(ghost.fromSceneId, finalFrom) || finalFrom;
      const finalTo = ghost.position || ghostPosition(finalFrom, ghost, index);
      const to = resolveGhostPosition(ghost, index, finalTo) || finalTo;
    ...
    if (dragEdge) {
      ...
      const finalFrom = model.layout.positions[dragEdge.fromSceneId] || dragEdge.fromPosition;
      const from = resolveNodePosition(dragEdge.fromSceneId, finalFrom) || finalFrom;
      if (from) {
    ...
    if (renderOptions.wireHits === false) return;
  ...
  function positionKey(prefix, value) {
    return `${prefix}:${String(value || "")}`;
  }

  function animationPoint(position) {
    return {
  ...
  function shouldAnimatePosition(start, end) {
    if (!start || !end) return false;
    const dx = Number(start.x || 0) - Number(end.x || 0);
    const dy = Number(start.y || 0) - Number(end.y || 0);
    const distance = Math.hypot(dx, dy);
    return distance >= 1 && distance <= 900;
  }

  function interpolatePosition(start, end, progress) {
    if (!start || !end) return end;
    return {
      x: Number(start.x || 0) + (Number(end.x || 0) - Number(start.x || 0)) * progress,
      y: Number(start.y || 0) + (Number(end.y || 0) - Number(start.y || 0)) * progress,
    };
  }

  function layoutEase(progress) {
    const t = Math.max(0, Math.min(1, Number(progress || 0)));
    return 1 - Math.pow(1 - t, 3);
  }

  function buildEdgeAnimationState(model, ghosts = []) {
    if (suppressLayoutAnimation) return null;
    const nodeStartBySceneId = new Map();
    const ghostStartByKey = new Map();

    (model.nodes || []).forEach((node) => {
      const sceneId = Number(node && node.sceneId || 0);
      const finalPosition = node && node.position ? animationPoint(node.position) : null;
      const previous = previousNodePositions.get(positionKey("node", sceneId));
      if (!shouldAnimatePosition(previous, finalPosition)) return;
      nodeStartBySceneId.set(sceneId, animationPoint(previous));
    });

    ghosts.forEach((ghost) => {
      const key = ghostAnimationKey(ghost);
      const finalPosition = ghost && ghost.position ? animationPoint(ghost.position) : null;
      const previous = previousGhostPositions.get(key);
      if (!shouldAnimatePosition(previous, finalPosition)) return;
      ghostStartByKey.set(key, animationPoint(previous));
    });

    return nodeStartBySceneId.size || ghostStartByKey.size
      ? { nodeStartBySceneId, ghostStartByKey }
      : null;
  }

  function edgeRenderOptions(animation, rawProgress) {
    const progress = layoutEase(rawProgress);
    return {
      wireHits: rawProgress >= 1,
      nodePosition(sceneId, finalPosition) {
        const start = animation.nodeStartBySceneId.get(Number(sceneId || 0));
        return start ? interpolatePosition(start, finalPosition, progress) : finalPosition;
      },
      ghostPosition(ghost, index, finalPosition) {
        const start = animation.ghostStartByKey.get(ghostAnimationKey(ghost));
        return start ? interpolatePosition(start, finalPosition, progress) : finalPosition;
      },
    };
  }
  ...
  function cancelEdgeAnimation() {
    if (!edgeAnimationFrame) return;
    window.cancelAnimationFrame(edgeAnimationFrame);
    edgeAnimationFrame = 0;
  }

  function startEdgeAnimation(svg, model, ghosts, testResult, animation) {
    if (!animation) return;
    const startedAt = window.performance.now();
    const tick = (now) => {
      const progress = Math.min(1, (now - startedAt) / LAYOUT_ANIMATION_MS);
      renderEdges(svg, model, ghosts, testResult, edgeRenderOptions(animation, progress));
      if (progress < 1) {
        edgeAnimationFrame = window.requestAnimationFrame(tick);
      } else {
        edgeAnimationFrame = 0;
      }
    };
  ...
  function ghostAnimationKey(ghost) {
    return [
      "ghost",
      ghost && ghost.pathId,
      ghost && ghost.fromSceneId,
      ghost && ghost.sceneId,
      ghost && ghost.direction,
    ].map((part) => String(part || "")).join(":");
  }

  function applyPositionAnimation(node, key, position, cache) {
    const next = {
      x: Number(position && position.x || 0),
      y: Number(position && position.y || 0),
    };
    ...
    cache.set(key, next);
    if (suppressLayoutAnimation || !previous) return;
    const dx = Math.round(Number(previous.x || 0) - next.x);
    const dy = Math.round(Number(previous.y || 0) - next.y);
    if (Math.abs(dx) < 1 && Math.abs(dy) < 1) return;
    if (Math.hypot(dx, dy) > 900) return;
    node.style.transform = `translate(${dx}px, ${dy}px)`;
    window.requestAnimationFrame(() => {
      node.classList.add("layoutMoving");
      window.requestAnimationFrame(() => {
        node.style.transform = "translate(0, 0)";
      });
    });
    window.setTimeout(() => {
      node.classList.remove("layoutMoving");
      node.style.removeProperty("transform");
    }, LAYOUT_ANIMATION_MS + 180);
  }

  function prunePositionCache(cache, keys) {
  ...
  function createNode(nodeModel, members, model, testResult = null) {
    ...
    const showOverrideButton = testMode && canOverrideTestStatus(testStatus);
    ...
    applyPositionAnimation(node, positionKey("node", sceneId), pos, previousNodePositions);
    ...
    node.innerHTML = `
      ...
      ${showOverrideButton ? `<button class="testOverrideNodeButton" type="button" data-node-test-override title="Alleen voor deze test: start vanaf deze node">Override</button>` : ""}
    ...
    const overrideButton = node.querySelector("[data-node-test-override]");
    if (overrideButton) {
      overrideButton.addEventListener("mousedown", (event) => {
        event.preventDefault();
        event.stopPropagation();
      });
  ...
  function ghostPositionFromPath(ghost, model) {
    ...
    const anchorRendered = model.baseLayout && model.baseLayout.positions
      ? model.baseLayout.positions[Number(ghost.fromSceneId || 0)]
      : model.layout.positions[Number(ghost.fromSceneId || 0)];
  ...
  function createGhostNode(ghost, pos) {
    ...
    node.style.left = `${pos.x}px`;
    node.style.top = `${pos.y}px`;
    applyPositionAnimation(node, ghostAnimationKey(ghost), pos, previousGhostPositions);
      ...
      const move = (moveEvent) => {
        ...
        dragItems.forEach((item) => {
          ...
          item.paths.forEach((path) => {
            const manual = ensureManualLayout(pathKey(path));
            manual[item.id] = canvasToPathLocalPosition(model, path, nextPosition, item.id);
          });
        });
        if (dragItems.length) {
  ...
  function finishEdgeDrag(toSceneId, options = {}) {
    ...
    if (!hasFrom && dragEdge.fromPosition) manual[fromSceneId] = canvasToPathLocalPosition(model, path, dragEdge.fromPosition, fromSceneId);
    if (!hasTo && options.toPosition) manual[targetSceneId] = canvasToPathLocalPosition(model, path, options.toPosition, targetSceneId);
  ...
  async function saveDirtyPaths() {
    ...
    try {
      for (const payload of payloads) {
        const result = await api("/v0/paths/paths/upsert", { method: "POST", body: payload.body });
      ...
      for (const sceneId of dirtyCrossingSceneIds) {
        ...
        const result = await api("/v0/paths/crossing-thresholds/upsert", {
          method: "POST",
          body: {
            sceneId,
  ...
  async function archiveActivePath() {
    ...
    try {
      const result = await api("/v0/paths/archive", {
        method: "POST",
  ...
  async function deleteActivePath() {
    ...
    try {
      if (path.isActive !== false && !path.archivedAt) {
        await api("/v0/paths/archive", {
          method: "POST",
          body: { kind: "path", id: path.id },
        });
      }
      const result = await api("/v0/paths/delete", {
        method: "POST",
        body: { kind: "path", id: path.id },
      });
      if (result && result.state) state = normalizeEditorState(result.state);
  ...
  function focusPathInView(pathOrKey = activeKey(), options = {}) {
    ...
    scrollCanvasTo(
      Math.max(0, centerX * canvasZoom - outer.clientWidth / 2),
      Math.max(0, centerY * canvasZoom - outer.clientHeight / 2),
      options
    );
  }

  function scrollCanvasTo(left, top, options = {}) {
    const outer = $("canvasOuter");
    if (!outer) return;
    if (canvasScrollAnimation) {
      window.cancelAnimationFrame(canvasScrollAnimation);
      canvasScrollAnimation = 0;
    }
    if ((options.behavior || "smooth") === "auto") {
      outer.scrollTo({ left, top, behavior: "auto" });
      return;
    }
    ...
    if (Math.hypot(deltaLeft, deltaTop) < 2) return;
    const duration = Number(options.duration || LAYOUT_ANIMATION_MS);
    const startedAt = window.performance.now();
    const ease = (t) => 1 - Math.pow(1 - t, 3);
    const tick = (now) => {
      const progress = Math.min(1, (now - startedAt) / duration);
      const eased = ease(progress);
      outer.scrollLeft = startLeft + deltaLeft * eased;
      outer.scrollTop = startTop + deltaTop * eased;
      if (progress < 1) {
        canvasScrollAnimation = window.requestAnimationFrame(tick);
      } else {
        canvasScrollAnimation = 0;
      }
    };
  ...
  function wire() {
    ...
    $("pathSearch").addEventListener("input", (event) => {
      pathSearchQ = event.target.value;
      renderChips();
    });
    $("pathSearch").addEventListener("keydown", (event) => {
      if (event.key !== "Escape") return;
      pathSearchQ = "";
      event.target.value = "";
      renderChips();
    });
    $("showNeighborsToggle").checked = !!showNeighbors;
