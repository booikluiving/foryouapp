...
const DEFAULT_CAPTION_STYLE_PATH = path.join(__dirname, "..", "..", "..", "data", "teleprompter-caption-style.json");
...
function mountTeleprompterParser(app, options = {}) {
  ...
  app.post("/api/teleprompter-parser/cue", (req, res) => {
    ...
    if (endSceneFromStage && shouldEndSceneFromCueAdvance({ cue, requestedIndex, preparedScene })) {
      clearAutoCameraReactionTimers();
      try {
        const endScene = endSceneFromStage({ reason: "teleprompter_cue_end_card" });
        res.json({
  ...
  app.post("/api/teleprompter-parser/end-scene", async (_req, res) => {
    clearAutoCameraReactionTimers();
  ...
  app.post("/admin/teleprompter-parser/auto-camera", requireAdmin, (req, res) => {
    const body = req.body && typeof req.body === "object" ? req.body : {};
  ...
  app.post("/admin/teleprompter-parser/prepare", requireAdmin, (req, res) => {
    clearAutoCameraReactionTimers();
    const body = req.body && typeof req.body === "object" ? req.body : {};
    const result = store.prepareScene(body);
    broadcast("prepare");
    res.json({
      ok: true,
  ...
  app.post("/admin/teleprompter-parser/ready", requireAdmin, (req, res) => {
    clearAutoCameraReactionTimers();
    const body = req.body && typeof req.body === "object" ? req.body : {};
  ...
  app.post("/admin/teleprompter-parser/reveal", requireAdmin, (_req, res) => {
    clearAutoCameraReactionTimers();
  ...
  function prepare(input = {}) {
    clearAutoCameraReactionTimers();
    const result = store.prepareScene(input);
    broadcast("prepare");
