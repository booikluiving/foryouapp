...
const TOOL_PORT = Number(process.env.CAMERA_CONTROL_PORT || 3110);
const TOOL_HOST = normalizeBindHost(process.env.CAMERA_CONTROL_HOST || "127.0.0.1");
const CONTROL_TOKEN = String(process.env.CAMERA_CONTROL_TOKEN || "").trim();
const ALLOW_REMOTE = parseBooleanLike(process.env.CAMERA_CONTROL_ALLOW_REMOTE || "0");
const OSC_PORT = Number(process.env.CAMERA_CONTROL_OSC_PORT || 53100);
const OSC_LISTEN_ADDRESS = normalizeBindHost(process.env.CAMERA_CONTROL_OSC_LISTEN_ADDRESS || "127.0.0.1");
const CAMERA_TIMEOUT_MS = Number(process.env.CAMERA_CONTROL_TIMEOUT_MS || 1800);
const CAMERA_POLL_MS = Number(process.env.CAMERA_CONTROL_POLL_MS || 2000);
const WS_RECONNECT_BASE_MS = Number(process.env.CAMERA_CONTROL_WS_RECONNECT_BASE_MS || 1000);
...
function buildCameraConfig() {
  const defaults = [
    ["cam1", "CAM1", process.env.CAMERA_CONTROL_CAM1_HOST || "192.168.1.165"],
    ["cam2", "CAM2", process.env.CAMERA_CONTROL_CAM2_HOST || "192.168.1.166"],
    ["cam3", "CAM3", process.env.CAMERA_CONTROL_CAM3_HOST || "192.168.1.167"],
  ];
  return Object.fromEntries(defaults.map(([id, label, host]) => [id, {
    id,
    label,
    host: String(host).trim(),
    previewUrl: String(process.env[`CAMERA_CONTROL_${id.toUpperCase()}_PREVIEW_URL`] || "").trim(),
    tally: "none",
    online: false,
    restOk: false,
    wsStatus: "idle",
...
function requireSafeBind(kind, host, options = {}) {
  ...
  const suffix = options.allowToken
    ? "Set CAMERA_CONTROL_TOKEN to allow remote HTTP binding."
    : "Set CAMERA_CONTROL_ALLOW_REMOTE=1 to allow remote OSC binding.";
