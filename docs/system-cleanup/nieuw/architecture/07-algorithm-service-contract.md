...
- beheert persisted scoring-config en geeft een read-only config snapshot aan Runtime bij start run.
- beheert per bestaande `showRunId` een scoring context/score state voor debug en scoreberekening.
...
Algorithm heeft een losse UI onder `/algorithm/`. Die UI is geen operator-runtimepagina.

De UI mag tonen en aanpassen:

- heart, bored en chat/message weights;
- tijdcorrectie/duurnormalisatie;
- publieksgrootte-normalisatie;
- label affinity;
- character affinity;
- diversity, exploration, retry en scene-repeat penalty;
- neutral predicted score;
- service health;
- scoring contexts per bestaande `showRunId`;
- observations per `situationRunId`;
- observed score, predicted scores, confidence en reasons;
- top personages, labels en situaties afgeleid uit de scorefeed;
- simulaties die een observation doorrekenen zonder Runtime te schrijven.

De UI mag niet tonen of beheren:

- prompt settings;
- `preparedNext`;
- `currentOrder`;
- path locked/available editors;
- start/stop run;
- start/stop situation;
- Runtime shuffle/orderregels.
...
Een scoring context wordt alleen gemaakt voor een bestaande Runtime-`showRunId` met een catalogus- of run snapshot. Algorithm verzint geen `showRunId`.
...
- `GET /v0/algorithm/config`
- `PATCH /v0/algorithm/config`
- `PUT /v0/algorithm/config`
- `GET /v0/algorithm/config-snapshot`

Scoring context/score state:

- `GET /v0/algorithm/scoring-contexts`
- `POST /v0/algorithm/scoring-contexts`
- `GET /v0/algorithm/scoring-contexts/:showRunId`
- `GET /v0/algorithm/scoring-contexts/:showRunId/scores`
- `POST /v0/algorithm/events/situation-observed`
- `POST /v0/algorithm/simulate`

De oude naam `POST /v0/algorithm/runs` mag tijdelijk blijven als backwards-compatible alias, maar nieuwe UI en documentatie gebruiken `scoring-contexts`.
