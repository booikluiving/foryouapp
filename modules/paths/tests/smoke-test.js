...
async function fetchJsonAt(baseUrl, pathname, options) {
...
async function fetchJson(pathname, options) {
  return fetchJsonAt(`http://127.0.0.1:${PORT}`, pathname, options);
}
...
async function waitForHealth(child, logs, baseUrl = `http://127.0.0.1:${PORT}`, serviceName = "paths") {
  const deadline = Date.now() + 8000;
  while (Date.now() < deadline) {
    if (child.exitCode != null) throw new Error(`${serviceName} server exited early (${child.exitCode}): ${logs.join("")}`);
    try {
      return await fetchJsonAt(baseUrl, "/health");
    } catch (_err) {
      await new Promise((resolve) => setTimeout(resolve, 150));
    }
  }
  throw new Error(`${serviceName} server did not become healthy: ${logs.join("")}`);
}
...
async function main() {
  CATALOG_PORT = await findFreePort();
  ...
  await fs.copyFile(DEFAULT_STORE_PATH, TEST_STORE_PATH);
  const beforeHashes = await protectedHashes();
  const defaultStoreBefore = await hashPathState(DEFAULT_STORE_PATH);
  const catalogLogs = [];
  const catalogChild = spawn(process.execPath, [path.resolve(TEST_DIR, "../../catalog/server/server.js")], {
    cwd: APP_ROOT,
    env: {
      ...process.env,
  ...
  catalogChild.stdout.on("data", (chunk) => catalogLogs.push(String(chunk)));
  catalogChild.stderr.on("data", (chunk) => catalogLogs.push(String(chunk)));
  const catalogBaseUrl = `http://127.0.0.1:${CATALOG_PORT}`;
  await waitForHealth(catalogChild, catalogLogs, catalogBaseUrl, "catalog");
  ...
  const child = spawn(process.execPath, [path.resolve(TEST_DIR, "../server/server.js")], {
    ...
    env: {
      ...
      V2_PATHS_CATALOG_URL: catalogBaseUrl,
  ...
  try {
    ...
    assert.equal(snapshot.source.seededFrom.type, "v2-owned-import");
    assert(!JSON.stringify(snapshot.source.seededFrom).includes("legacy/data/live.sqlite"), "paths source must not expose legacy sqlite paths");
    ...
    assert(editorHtml.includes('href="/universe"'), "editor should link to Universe in the same tab");
    assert(!editorHtml.includes('href="/universe" target="_blank"'), "editor Universe link should not open a new tab");
    assert(editorHtml.includes("/editor/paden-graph.js"), "editor should load graph code from the stable editor URL");
    assert(editorHtml.includes("/editor/layout-relaxer.js"), "editor should load layout relaxer");
    const editorGraph = await fetchText("/editor/paden-graph.js");
    assert(editorGraph.includes("root.PadenGraph = factory()"), "editor graph URL should serve browser graph code");
    assert(editorGraph.includes("buildPathSceneStatuses"), "editor graph URL should serve rules-engine graph implementation");
    ...
    assert(universeHtml.includes('href="/editor/"'), "Universe page should link back to the Paths editor");
