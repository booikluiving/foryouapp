(function () {
  ...
  function createSkyRenderer(options) {
    ...
    const onSceneClick = typeof options.onSceneClick === "function" ? options.onSceneClick : null;
    ...
    function handleSceneClick(sceneId, context = {}) {
      if (!onSceneClick) return false;
      return onSceneClick({
        sceneId: Number(sceneId || 0),
        pathId: context.pathId ? String(context.pathId) : "",
        view: state.view,
        mode: state.mode,
      }) === true;
    }
    ...
    function networkEdgeClass(edge) {
      ...
      if (fromStatus === "current" || toStatus === "current") classes.push("current-edge");
      if (["current", "next", "available", "played"].includes(fromStatus) || ["current", "next", "available", "played"].includes(toStatus)) {
        classes.push("runtime-edge");
      }
    ...
    function bindNetworkNodeEvents(target, visibleStar, node) {
      ...
      target.addEventListener("click", (event) => {
        if (!handleSceneClick(node && node.sceneId, { network: true })) return;
    ...
    function drawLooseScenes(root) {
      ...
      state.model.looseSceneIds.forEach((sceneId, index) => {
        ...
        dot.addEventListener("click", (event) => {
          event.stopPropagation();
          if (handleSceneClick(sceneId)) return;
    ...
    function bindStarEvents(target, visibleStar, pathItem, sceneId) {
      ...
      target.addEventListener("click", (event) => {
        event.stopPropagation();
        if (handleSceneClick(sceneId, { pathId: pathItem.id })) return;
        if (state.mode === "focus" && state.selectedPathId === String(pathItem.id)) {
