(function initPadenGraph(root, factory) {
  ...
  function buildPathSceneStatuses({ paths = [], scenes = [], playedSceneIds = [], seedSceneIds = [], crossingThresholds = [] } = {}) {
    ...
    const seedSet = new Set(normalizeIdList(seedSceneIds));
    ...
    active.forEach((path) => {
      ...
      seedSet.forEach((sceneId) => {
        if (sceneIdSet.has(sceneId) && playedSet.has(sceneId) && !queue.includes(sceneId)) queue.push(sceneId);
      });
