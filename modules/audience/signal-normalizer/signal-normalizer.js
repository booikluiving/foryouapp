...
function buildAlgorithmInput({ showRunId, situationRunId, signals = [], createdAtDate = new Date() }) {
  ...
  return {
    ...
    startedAt: linked.length ? linked[0].link.startedAt || null : null,
    ...
    audience: {
      activeClients: Math.max(1, uniqueCount(linked.map((signal) => signal.clientId || signal.clientKey || signal.sessionId))),
    ...
    signalRefs: linked.map((signal) => ({
      ...
      clientKey: signal.clientKey || null,
      isBot: !!signal.isBot,
