...
Catalog Service V0 bestaat al onder `modules/catalog/`.
...
- Werk alleen onder `modules/catalog/` en, alleen waar nodig, `shared/`.
- Raak V1 app-code, V1 catalogusbestanden en `legacy/data/live.sqlite*` niet aan.
- Gebruik `legacy/public/algoritme.html` en de oude serverroutes alleen read-only als referentie.
...
- Gebruik de gedeelde V2 stylesheet `shared/ui/foryou-v2.css` als visuele basis.
...
- relevante oude routes in `legacy/server.js`, alleen om workflow en velden te begrijpen.
...
Het is acceptabel om de eerste V2 catalog store als JSON files onder `modules/catalog/db/` te maken, als dit goed getest en uitlegbaar is. Gebruik geen verborgen complexiteit als dat nog niet nodig is.
