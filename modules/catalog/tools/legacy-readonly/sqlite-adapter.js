...
const LEGACY_ROOT = path.resolve(process.env.V2_LEGACY_ROOT || path.join(V2_ROOT, "legacy"));
const DEFAULT_LEGACY_SQLITE_PATH = path.join(LEGACY_ROOT, "data", "live.sqlite");
...
function rebaseManifestPaths(value) {
  const oldRoot = path.join(V2_ROOT, "modules", "catalog", "legacy-readonly", "snapshots");
  const newRoot = path.join(V2_ROOT, "modules", "catalog", "tools", "legacy-readonly", "snapshots");
  if (typeof value === "string") return value.startsWith(oldRoot) ? path.join(newRoot, path.relative(oldRoot, value)) : value;
  if (Array.isArray(value)) return value.map(rebaseManifestPaths);
  if (value && typeof value === "object") {
    return Object.fromEntries(Object.entries(value).map(([key, item]) => [key, rebaseManifestPaths(item)]));
  }
  return value;
}

function readManifest(manifestPath) {
  return rebaseManifestPaths(JSON.parse(fs.readFileSync(manifestPath, "utf8")));
}
