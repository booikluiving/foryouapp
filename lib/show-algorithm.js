...
function normalizeAlgorithmSettings(input = {}) {
  ...
  return {
    ...
    diversityWeight: Number(clampFloat(src.diversityWeight, 0, 5, DEFAULT_ALGORITHM_SETTINGS.diversityWeight).toFixed(2)),
    explorationWeight: Number(clampFloat(src.explorationWeight, 0, 5, DEFAULT_ALGORITHM_SETTINGS.explorationWeight).toFixed(2)),
    retryWeight: Number(clampFloat(src.retryWeight, 0, 2, DEFAULT_ALGORITHM_SETTINGS.retryWeight).toFixed(2)),
    sceneRepeatPenalty: Number(clampFloat(src.sceneRepeatPenalty, 0, 5, DEFAULT_ALGORITHM_SETTINGS.sceneRepeatPenalty).toFixed(2)),
  };
}
...
function buildAlgorithmOrder({ scenes = [], runs = [], settings = {}, catalog = null, preparedNext = null, lockedQueue = [] } = {}) {
  ...
  for (const queueEntry of requestedLockedQueue) {
    ...
    if (!queueScene || !isContextReadyForLockedEntry(queueScene, queueEntry.position)) continue;
