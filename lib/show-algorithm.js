...
function buildAlgorithmOrder({ scenes = [], runs = [], settings = {}, catalog = null } = {}) {
  ...
  for (const scene of available) {
    ...
    if (validation.ok) {
      ...
      invalid.push(buildOrderEntry(scene, {
        invalid: true,
  ...
  const calibrationEntries = calibrationScenes.map((scene) => {
    const block = contextBlockForScene(scene);
    return buildOrderEntry(scene, {
      calibration: true,
      played: played.has(scene.id),
  ...
  if (calibration.active) {
    ...
    upcoming = playableScenes
      ...
      .map((scene) => {
        const block = contextBlockForScene(scene);
        return block ? buildBlockedContextEntry(scene, { score: 0 }) : buildOrderEntry(scene, {
          score: 0,
    ...
    if (firstBlockedEntry) {
      ...
      if (contextScene) {
        contextEntry = buildOrderEntry(contextScene, {
  ...
  const hiddenPlayed = cycleComplete ? [] : playableScenes
    ...
    .map((scene) => buildOrderEntry(scene, {
      played: true,
      hidden: true,
...
function pickRecommendation({ scenes = [], runs = [], settings = {}, catalog = null } = {}) {
  const order = buildAlgorithmOrder({ scenes, runs, settings, catalog });
  const recommendationEntry = order.next || order.upcoming[0] || null;
  if (!recommendationEntry) {
    const reason = order.playableCount
    ...
    return {
      ...
      entityScores: order.entityScores,
      candidates: [],
    };
  }
  ...
  return {
    ...
    candidates: [recommendationEntry]
      .concat(order.upcoming.filter((entry) => entry.sceneId !== recommendationEntry.sceneId))
      .slice(0, 12)
      .map((candidate) => ({
