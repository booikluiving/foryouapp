...
function normalizePathThresholdsForEdges(thresholds = [], sceneIds = [], edges = []) {
  const ids = normalizeIdList(sceneIds);
  const outgoingBySource = new Map(ids.map((id) => [id, new Set()]));
  for (const edge of Array.isArray(edges) ? edges : []) {
    ...
    if (!outgoingBySource.has(fromSceneId) || !outgoingBySource.has(toSceneId) || fromSceneId === toSceneId) continue;
    outgoingBySource.get(fromSceneId).add(toSceneId);
  }
  return normalizePathThresholds(thresholds)
    .map((threshold) => {
      const outgoingCount = outgoingBySource.get(threshold.sourceSceneId)
        ? outgoingBySource.get(threshold.sourceSceneId).size
        : 0;
      if (outgoingCount <= 1) return null;
      const requiredCount = clampInt(threshold.requiredCount, 1, outgoingCount, outgoingCount);
...
function buildPathSceneStatuses(paths = [], sceneById = new Map(), played = new Set(), crossingThresholds = []) {
  ...
  const crossingThresholdBySceneId = new Map(normalizeCrossingThresholdsForPaths(crossingThresholds, active)
    .map((threshold) => [threshold.sceneId, threshold]));
  ...
  for (const path of active) {
    ...
    const thresholdBySourceSceneId = new Map(effectivePathThresholds(path)
      .map((threshold) => [threshold.sourceSceneId, threshold]));
    const thresholdGroups = [];
    for (const [sourceSceneId, targetIds] of requiredOutgoingBySceneId.entries()) {
      const targetSceneIds = uniqueNormalizedIds(targetIds);
      const threshold = thresholdBySourceSceneId.get(sourceSceneId);
      if (!threshold || targetSceneIds.length <= 1) continue;
      const completedSceneIds = targetSceneIds.filter((targetId) => played.has(targetId));
      ...
      thresholdGroups.push({
        ...
        satisfied,
        optionalSceneIds: satisfied ? targetSceneIds.filter((targetId) => !played.has(targetId)) : [],
      });
    }
    ...
    for (const sceneId of sceneIds) {
      ...
      const activeSideSourceIds = sideIncoming.filter((sourceSceneId) => played.has(sourceSceneId));
      const expiredSideSourceIds = sideIncoming.filter((sourceSceneId) => (
        played.has(sourceSceneId)
        && (requiredOutgoingBySceneId.get(sourceSceneId) || []).some((targetId) => played.has(targetId))
      ));
      const missingSideSourceIds = sideIncoming.filter((sourceSceneId) => !played.has(sourceSceneId));
      const optionalPredecessorIds = requiredIncoming.filter((predecessorId) => (
        !played.has(predecessorId) && optionalGroupsByTarget.has(predecessorId)
      ));
      ...
      const completedPredecessorIds = incoming.filter((predecessorId) => played.has(predecessorId) && !expiredSideSourceIds.includes(predecessorId));
      ...
      const incomingBlockingThresholds = thresholdGroups.filter((group) => (
        !group.satisfied && incoming.some((predecessorId) => group.targetSceneIds.includes(predecessorId))
      ));
      ...
      status.pathDetails.push({
        ...
        expiredSideSceneIds: uniqueNormalizedIds(sideOutgoing.filter((targetId) => {
          if (!played.has(sceneId)) return false;
          return (requiredOutgoingBySceneId.get(sceneId) || []).some((nextId) => played.has(nextId)) && !played.has(targetId);
        })),
        ...
        optional: (optionalSceneGroups.length > 0 || activeSideSourceIds.length > 0) && !expiredSideSourceIds.length && !played.has(sceneId),
        expiredOptional: expiredSideSourceIds.length > 0 && !played.has(sceneId),
        ready: missingPredecessorIds.length === 0,
  ...
  for (const status of statusBySceneId.values()) {
    ...
    status.blockingThresholds = uniqueThresholdGroupStatuses(status.pathDetails.flatMap((detail) => detail.blockingThresholds || []));
    ...
    const crossingRouteKeys = new Set();
    for (const detail of status.pathDetails) {
      const pathId = normalizeId(detail.pathId);
      for (const predecessorId of normalizeIdList(detail.requiredPredecessorIds || [])) {
        const key = `${pathId}:${predecessorId}`;
        if (!pathId || crossingRouteKeys.has(key)) continue;
        crossingRouteKeys.add(key);
        crossingRoutes.push({
          pathId,
          pathName: normalizeText(detail.pathName, 140) || `Pad ${pathId}`,
          color: normalizeText(detail.color, 40),
          fromSceneId: predecessorId,
          toSceneId: status.sceneId,
          completed: played.has(predecessorId),
        });
      }
    }
    const crossingThreshold = crossingThresholdBySceneId.get(status.sceneId) || null;
    if (crossingThreshold && crossingRoutes.length > 1) {
      const incomingCount = crossingRoutes.length;
      const requiredCount = clampInt(crossingThreshold.requiredCount, 1, incomingCount, incomingCount);
      const completedCount = crossingRoutes.filter((route) => route.completed).length;
      const satisfied = completedCount >= requiredCount;
      const optionalPredecessorIds = satisfied
        ? crossingRoutes.filter((route) => !route.completed).map((route) => route.fromSceneId)
        : [];
      status.crossingThreshold = {
        sceneId: status.sceneId,
        incomingCount,
        requiredCount,
        completedCount,
        satisfied,
        routes: crossingRoutes,
        optionalPredecessorIds: uniqueNormalizedIds(optionalPredecessorIds),
      };
      status.crossingIncomingCount = incomingCount;
      status.crossingRequiredCount = requiredCount;
      status.crossingCompletedCount = completedCount;
      status.crossingSatisfied = satisfied;
      if (satisfied) {
        status.crossingOptionalPredecessorIds = uniqueNormalizedIds(optionalPredecessorIds);
        status.optionalPredecessorIds = uniqueNormalizedIds([
          ...status.optionalPredecessorIds,
          ...status.crossingOptionalPredecessorIds,
        ]);
        const optionalSet = new Set(status.crossingOptionalPredecessorIds);
        status.missingPredecessorIds = uniqueNormalizedIds(status.missingPredecessorIds.filter((id) => !optionalSet.has(Number(id))));
        status.requiredMissingPredecessorIds = uniqueNormalizedIds(status.requiredMissingPredecessorIds.filter((id) => !optionalSet.has(Number(id))));
      } else {
        status.blockingCrossingThreshold = status.crossingThreshold;
      }
    }
    status.ready = status.missingPredecessorIds.length === 0 && !status.blockingCrossingThreshold;
  }
...
function buildAlgorithmOrder({ scenes = [], runs = [], settings = {}, catalog = null, preparedNext = null, lockedQueue = [] } = {}) {
  ...
  const pathBlockForScene = (scene) => {
    ...
    const blockingCrossingThreshold = status.blockingCrossingThreshold && !status.blockingCrossingThreshold.satisfied
    ...
    return {
      ...
      reason: expiredSide.length
        ...
        : blockingThreshold
        ? `Wacht op ${blockingThreshold.requiredCount} van ${blockingThreshold.outgoingCount} situaties in deze groep; ${blockingThreshold.completedCount} gespeeld.`
        : blockingCrossingThreshold
        ? `Wacht op ${blockingCrossingThreshold.requiredCount} van ${blockingCrossingThreshold.incomingCount} paden naar deze kruising; ${blockingCrossingThreshold.completedCount} vrijgegeven.`
        : status.isMerge
  ...
  const hasUnplayedOutsideActive = playableScenes.some((scene) => {
    ...
    const status = pathStatusForScene(scene);
    return !(status && status.expiredOptional);
  });
