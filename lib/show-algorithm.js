...
function buildAlgorithmOrder({ scenes = [], runs = [], settings = {}, catalog = null } = {}) {
  ...
  const exposedNext = activeSceneId ? null : next;
...
function pickRecommendation({ scenes = [], runs = [], settings = {}, catalog = null, order = null } = {}) {
  ...
  const recommendationEntry = safeOrder.activeSceneId
    ? null
    : safeOrder.next || safeOrder.upcoming[0] || null;
  if (!recommendationEntry) {
    const reason = safeOrder.activeSceneId
      ? "Er is al een situatie actief."
      : safeOrder.playableCount
