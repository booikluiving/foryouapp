...
function createDropboxCatalogSync(options = {}) {
  ...
  function sql() {
    ...
    statements = {
      ...
      scenes: database.prepare(`SELECT id, title, sort_order, character_count, character_slots_json, character_ids_json, situation_ids_json, environment_id, environment_mode, context_scene_id, prompt_override, is_active, archived_at, created_at, updated_at, COALESCE(external_id, '') AS external_id FROM algorithm_scenes ORDER BY sort_order ASC, id ASC`),
      ...
      getSceneById: database.prepare(`SELECT id, title, sort_order, character_count, character_slots_json, character_ids_json, situation_ids_json, environment_id, environment_mode, context_scene_id, prompt_override, is_active, archived_at, created_at, updated_at FROM algorithm_scenes WHERE id = ?`),
  ...
  function importScene(meta, text) {
    ...
    if (row) {
      ...
      database.prepare(`UPDATE algorithm_scenes SET sort_order = ?, character_count = ?, character_slots_json = ?, character_ids_json = ?, situation_ids_json = ?, environment_id = ?, environment_mode = ?, context_scene_id = ?, is_active = ?, archived_at = ?, updated_at = ? WHERE id = ?`)
        .run(
    ...
    const insert = database.prepare(`INSERT INTO algorithm_scenes (title, sort_order, character_count, character_slots_json, character_ids_json, situation_ids_json, environment_id, environment_mode, context_scene_id, prompt_override, is_active, archived_at, created_at, updated_at, external_id) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, '', ?, ?, ?, ?, ?)`)
      .run(
        title,
